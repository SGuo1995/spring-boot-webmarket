$(function() {
    ...
    var pageSize = 10;
    // 获取奖品列表的URL
    var listUrl = '/javao2o/frontend/listawardsbyshop';
    // 兑换奖品的URL
    var exchangeUrl = '/javao2o/frontend/adduserawardmap';
    var pageNum = 1;
    // 从地址栏URL里获取shopId
            ...
            if (data.success) {
                ...
                data.awardList.map(function(item, index) {
                    ...
                    if (data.totalPoint != undefined) {
                        // 若用户有积分，则显示领取按钮
                        html += '<span>Click to redeem</span></div></div>'
                    } else {
                        html += '</div></div>'
                    }
                });
                ...
                if (data.totalPoint != undefined) {
                    // 若用户在该店铺有积分，则显示
                    canProceed = true;
                    $("#title").text('Total Points' + data.totalPoint);
        ...
        .on(
            ...
            function(e) {
                ...
                if (canProceed
                    ...
                    $.toast('Insufficient points or unauthorized');
                }
            });
