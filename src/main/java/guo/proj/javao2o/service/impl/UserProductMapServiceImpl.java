...
import guo.proj.javao2o.dao.UserProductMapDao;
...
import guo.proj.javao2o.enums.UserProductMapStateEnum;
import guo.proj.javao2o.exceptions.UserProductMapOperationException;
import guo.proj.javao2o.service.UserProductMapService;
    ...
    public UserProductMapExecution addUserProductMap(UserProductMap userProductMap)
        ...
        if (userProductMap != null && userProductMap.getUser().getUserId() != null
            ...
            try {
                ...
                if (effectedNum <= 0) {
                    throw new UserProductMapOperationException("Failed to add order of user");
                }
                // 若本次消费能够积分
                if (userProductMap.getPoint() != null && userProductMap.getPoint() > 0) {
                    ...
                    if (userShopMap != null && userShopMap.getUserShopId() != null) {
                        ...
                        if (effectedNum <= 0) {
                            throw new UserProductMapOperationException("Failed to update points");
                        }
                    } else {
                        ...
                        if (effectedNum <= 0) {
                            throw new UserProductMapOperationException("Failed to create point info");
                        }
                    }
                }
                return new UserProductMapExecution(UserProductMapStateEnum.SUCCESS, userProductMap);
            } catch (Exception e) {
                throw new UserProductMapOperationException("Failed to add auth:" + e.toString());
            }
        } else {
