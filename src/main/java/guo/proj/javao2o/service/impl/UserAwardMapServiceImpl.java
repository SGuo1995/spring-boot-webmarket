...
import guo.proj.javao2o.dao.UserAwardMapDao;
import guo.proj.javao2o.dao.UserShopMapDao;
import guo.proj.javao2o.dto.UserAwardMapExecution;
import guo.proj.javao2o.entity.UserAwardMap;
import guo.proj.javao2o.entity.UserShopMap;
import guo.proj.javao2o.enums.UserAwardMapStateEnum;
import guo.proj.javao2o.exceptions.UserAwardMapOperationException;
import guo.proj.javao2o.service.UserAwardMapService;
                ...
                if (userAwardMap.getPoint() != null && userAwardMap.getPoint() > 0) {
                    ...
                    if (userShopMap != null) {
                        ...
                        if (userShopMap.getPoint() >= userAwardMap.getPoint()) {
                            ...
                            throw new UserAwardMapOperationException("Unable to redeem (insufficient points)");
                        }
                    } else {
                        // 在店铺没有积分，则抛出异常
                        throw new UserAwardMapOperationException("Sorry, you haven't gained any points here");
                    }
                }
                // 插入礼品兑换信息
                effectedNum = userAwardMapDao.insertUserAwardMap(userAwardMap);
                if (effectedNum <= 0) {
                    throw new UserAwardMapOperationException("Failed to redeem rewards");
                }
                return new UserAwardMapExecution(UserAwardMapStateEnum.SUCCESS, userAwardMap);
            } catch (Exception e) {
                throw new UserAwardMapOperationException("Failed to redeem rewards:" + e.toString());
            }
        } else {
